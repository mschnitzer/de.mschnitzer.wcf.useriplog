		...
		if ($row['count']) {
			...
			if (count($multiaccounts) && count($recipients)) {
				...
				foreach ($recipients as $recipient) {
				    ...
				    $sql = "SELECT conversationID FROM wcf".WCF_N."_conversation
				    		WHERE
				    			userID = ? AND
				    			subject = ? AND
				    			lastPostTime+5 > ?
				    ...
				    $statement->execute(array(
				    	$recipient,
				    	WCF::getLanguage()->getDynamicVariable('wcf.iplog.messages.newMultiAccount.title', array()),
				    	TIME_NOW,
				    ));
				    ...
				    $statement = WCF::getDB()->prepareStatement($sql);
				    $statement->execute(array($row['conversationID']));

				    // reset cache
				    UserStorageHandler::getInstance()->resetAll('unreadConversationCount');
				}
			}
		}
